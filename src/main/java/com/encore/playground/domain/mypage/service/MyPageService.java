...
public class MyPageService {
    ...
    public MyPageDto myPage(MemberGetMemberIdDto memberIdDto) {
        ...
        // 사용자가 작성한 feed를 가져온다.
        List<FeedListDto> feedListDto = feedService.getFeedListByMember(memberIdDto);
        // 사용자가 작성한 comment를 가져온다.
        List<CommentListDto> commentListDto = commentService.getCommentsByUser(memberIdDto);
        // 사용자가 작성한 question을 가져온다.
        List<QuestionDto> questionListDto = questionService.getQuestionListByMember(memberDto.getId());
        // 사용자가 작성한 answer를 가져온다.
        List<AnswerDto> answerListDto = answerService.getAnswerListByMember(memberDto.getId());
